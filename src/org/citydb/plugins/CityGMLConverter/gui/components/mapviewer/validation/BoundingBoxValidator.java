 ...
 */
package org.citydb.plugins.CityGMLConverter.gui.components.mapviewer.validation;
...
public class BoundingBoxValidator {
	...
	public BoundingBoxValidator(MapWindow map, ConfigImpl config) {
		this.map = map;
		this.config = config;
	...
	public ValidationResult validate(final BoundingBox bbox) {
		...
		// unknown srs
			...
			ValidatorDialog validator = new ValidatorDialog(bbox, Util.I18N.getString("map.dialog.title.error"), config);
			validator.addErrorMessage(Util.I18N.getString("map.dialog.label.error.noSRS"));
			validator.addBoundingBox();
			validator.addAction(Util.I18N.getString("map.dialog.label.error.noSRS.hint"), 
					isDBConnected ? null : Util.I18N.getString("map.dialog.label.note.text"));
		...
		// srs is known but not wgs84
			...
			if (isDBConnected) {
				...
				ValidatorDialog validator = new ValidatorDialog(bbox, Util.I18N.getString("map.dialog.title.error"), config);
				validator.addErrorMessage(Util.I18N.getString("map.dialog.label.error.wgs84"));
				validator.addBoundingBox();
				validator.addAction(Util.I18N.getString("map.dialog.label.error.wgs84.hint"), Util.I18N.getString("map.dialog.label.note.text"));
				validator.addTransformButtons();
				validator.showDialog();
		...
		// srs is wgs84...
			...
			if (!(bbox.getLowerLeftCorner().getX() != null && bbox.getLowerLeftCorner().getX() >= -180 && bbox.getLowerLeftCorner().getX() <= 180 && 
				...
				ValidatorDialog validator = new ValidatorDialog(bbox, Util.I18N.getString("map.dialog.title.error"), config);
				validator.addErrorMessage(Util.I18N.getString("map.dialog.label.error.range"));
				validator.addBoundingBox();
			...
			// ...but coordinate values are invalid
				...
				ValidatorDialog validator = new ValidatorDialog(bbox, Util.I18N.getString("map.dialog.title.error"), config);
				validator.addErrorMessage(Util.I18N.getString("map.dialog.label.error.noArea"));
				validator.addBoundingBox();
				validator.addOkButton();
				validator.showDialog();
			...
			// ...but bounding box is not visible on screen
				...
				ValidatorDialog validator = new ValidatorDialog(bbox, Util.I18N.getString("map.dialog.title.error"), config);
				validator.addErrorMessage(Util.I18N.getString("map.dialog.label.error.notVisible"));
				validator.addBoundingBox();
				validator.addOkButton();
				validator.showDialog();
	...
	private ValidationResult transformBoundingBox(BoundingBox bbox) {
		...
		try {
			if (!dbController.isConnected()) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						transform.setMessage(Util.I18N.getString("main.status.database.connect.label"));
					}
				});
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					transform.setMessage(Util.I18N.getString("map.dialog.label.transform"));
				}
			});
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					transform.setErrorMessage(Util.I18N.getString("map.dialog.label.error.transform"));
				}
			});
		} catch (DatabaseConfigurationException e) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					transform.setErrorMessage(Util.I18N.getString("map.dialog.label.error.db"));
				}
			});
		}
	...
	public final class ValidatorDialog extends JDialog {
		...
		ValidatorDialog(BoundingBox bbox, String title, ConfigImpl config) {
		...
		private void addBoundingBox() {
			...
			JLabel title = new JLabel(Util.I18N.getString("map.boundingBox.label"));
			...
			if (bbox.isSetSrs()) {
				...
				c.anchor = GridBagConstraints.EAST;
				bboxPanel.add(new JLabel(Util.I18N.getString("pref.db.srs.label.description")), c);				
				...
				c.anchor = GridBagConstraints.EAST;
				bboxPanel.add(new JLabel(Util.I18N.getString("pref.db.srs.label.srid")), c);
		...
		private void addAction(String action, String hint) {
			...
			if (hint != null) {
				JLabel note = new JLabel(Util.I18N.getString("map.dialog.label.note") + ':');
		...
		private void addSrsComboBox() {			
			...
			JLabel srsLabel = new JLabel(Util.I18N.getString("common.label.boundingBox.crs"));
		...
		private void addTransformButtons() {
			...
			JButton transform = new JButton(Util.I18N.getString("map.dialog.button.transform"));
			JButton skip = new JButton(Util.I18N.getString("map.dialog.button.skip"));
			JButton close = new JButton(Util.I18N.getString("map.dialog.button.close"));
		...
		private void addOkButton() {
			...
			JButton ok = new JButton(Util.I18N.getString("common.button.ok"));
			JButton close = new JButton(Util.I18N.getString("map.dialog.button.close"));
	...
	public final class TransformDialog extends JDialog {		
		...
		TransformDialog() {
			super(map, Util.I18N.getString("map.dialog.title.transform"), true);
			init();
		}
		...
		private void init() {
			...
			button = new JButton(Util.I18N.getString("common.button.ok"));
		...
		private void setErrorMessage(String message) {
			message = message.replaceAll("\\n", "<br/>");
			setTitle(Util.I18N.getString("map.dialog.title.transform.error"));
