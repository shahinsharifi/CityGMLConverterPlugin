 ...
 */
package org.citydb.plugins.CityGMLConverter.gui.components.mapviewer;
...
import org.citydb.plugins.CityGMLConverter.gui.components.bbox.BoundingBoxClipboardHandler;
import org.citydb.plugins.CityGMLConverter.gui.components.mapviewer.validation.BoundingBoxValidator;
import org.citydb.plugins.CityGMLConverter.gui.components.mapviewer.validation.BoundingBoxValidator.ValidationResult;
...
public class MapWindow extends JDialog implements EventHandler {
	...
	private MapWindow(ConfigImpl config) {
	...
	public static final synchronized MapWindow getInstance(ConfigImpl config) {
		if (instance == null)
	...
	public static final synchronized MapWindow getInstance(BoundingBoxListener listener, ConfigImpl config) {
				...
				try {
					...
					if (response.getStatus() == StatusCode.OK) {
						...
						if (response.getType() == ResponseType.LAT_LON)
							resultMsg = Util.I18N.getString("map.geocoder.search.latLon");
						else {
							String text = Util.I18N.getString("map.geocoder.search.result");
						...
						case OVER_QUERY_LIMIT:
							resultMsg = Util.I18N.getString("map.geocoder.search.overLimit");
							break;		
						case REQUEST_DENIED:
							resultMsg = Util.I18N.getString("map.geocoder.search.denied");
							break;		
						default:
							LOG.error("Fatal service response from geocoder: " + response.getException().getMessage());
							resultMsg = Util.I18N.getString("map.geocoder.search.fatal");
						}					
					}

					resultMsg += " (" + ((System.currentTimeMillis() - time) / 1000.0) + " " + Util.I18N.getString("map.geocoder.search.sec") + ")";
	...
	private void setSizeOnScreen() {
		...
		ViewController viewController = ObjectRegistry.getInstance().getViewController();		
		JFrame mWindow = viewController.getTopFrame();
		

		Integer x = mWindow.getLocation().x;
		Integer y = mWindow.getLocation().y;
		Integer width = mWindow.getSize().width;
		Integer height = mWindow.getSize().height;
	...
	private void doTranslation() {
		setTitle(Util.I18N.getString("map.window.title"));
		applyButton.setText(Util.I18N.getString("common.button.apply"));
		cancelButton.setText(Util.I18N.getString("common.button.cancel"));
		goButton.setText(Util.I18N.getString("map.button.go"));
		bboxTitel.setText(Util.I18N.getString("map.boundingBox.label"));
		showBBox.setText(Util.I18N.getString("map.boundingBox.show.button"));
		showBBox.setToolTipText(Util.I18N.getString("map.boundingBox.show.tooltip"));
		clearBBox.setText(Util.I18N.getString("map.boundingBox.clear.button"));
		clearBBox.setToolTipText(Util.I18N.getString("map.boundingBox.clear.tooltip"));
		copyBBox.setToolTipText(Util.I18N.getString("common.tooltip.boundingBox.copy"));
		pasteBBox.setToolTipText(Util.I18N.getString("common.tooltip.boundingBox.paste"));
		reverseTitle.setText(Util.I18N.getString("map.reverseGeocoder.label"));
		reverseInfo.setText("<html>" + Util.I18N.getString("map.reverseGeocoder.hint.label") + "</html>");
		helpTitle.setText(Util.I18N.getString("map.help.label"));
		helpText.setText("<html>" + Util.I18N.getString("map.help.hint") + "</html>");
		googleMapsButton.setText(Util.I18N.getString("map.google.label"));
	...
	public void handleEvent(Event event) throws Exception {
		if (event.getEventType() == GlobalEvents.SWITCH_LOCALE) {
			...
			if (e.getStatus() == ReverseGeocoderStatus.SEARCHING) {
				...
				switch (response.getStatus()) {
				case ZERO_RESULTS:
					info = Util.I18N.getString("map.reverseGeocoder.search.noResult");
					break;
				case OVER_QUERY_LIMIT:
					info = Util.I18N.getString("map.geocoder.search.overLimit");
					break;		
				case REQUEST_DENIED:
					info = Util.I18N.getString("map.geocoder.search.denied");
					break;		
				default:
					LOG.error("Fatal service response from reverse geocoder: " + response.getException().getMessage());
					info = Util.I18N.getString("map.geocoder.search.fatal");
				}
	...
	private final class BBoxPopupMenu extends JPopupMenu {
		...
		private void doTranslation() {
			copy.setText(Util.I18N.getString("common.popup.boundingBox.copy"));
			paste.setText(Util.I18N.getString("common.popup.boundingBox.paste"));
		}
	}
