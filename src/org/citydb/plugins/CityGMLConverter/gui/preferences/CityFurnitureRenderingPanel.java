...
*/
	...
	public CityFurnitureRenderingPanel(CityKMLExportPlugin plugin) {
		super(plugin);
		initGui();
	}

	private ColladaOptions getConfigColladaOptions() {
		return plugin.getConfig().getCityFurnitureColladaOptions();
	}

	private List<DisplayForm> getConfigDisplayForms() {
		return plugin.getConfig().getCityFurnitureDisplayForms();
	}

	private void setConfigDisplayForms(List<DisplayForm> displayForms) {
		plugin.getConfig().setCityFurnitureDisplayForms(displayForms);
	}

	@Override
	public String getTitle() {
		return Util.I18N.getString("pref.tree.kmlExport.cityFurnitureRendering");
	}
