...
public class TunnelRenderingPanel extends AbstractPreferencesComponent {
	...
	public TunnelRenderingPanel(CityKMLExportPlugin plugin) {
		super(plugin);
		initGui();
	}

	private ColladaOptions getConfigColladaOptions() {
		return plugin.getConfig().getTunnelColladaOptions();
	}

	private List<DisplayForm> getConfigDisplayForms() {
		return plugin.getConfig().getTunnelDisplayForms();
	}

	private void setConfigDisplayForms(List<DisplayForm> displayForms) {
		plugin.getConfig().setTunnelDisplayForms(displayForms);
	}

	@Override
	public String getTitle() {
		return Util.I18N.getString("pref.tree.kmlExport.tunnelRendering");
	}
