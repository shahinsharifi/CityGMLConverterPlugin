...
public class BridgeRenderingPanel extends AbstractPreferencesComponent {
	...
	public BridgeRenderingPanel(CityKMLExportPlugin plugin) {
		super(plugin);
		initGui();
	}

	private ColladaOptions getConfigColladaOptions() {
		return plugin.getConfig().getBridgeColladaOptions();
	}

	private List<DisplayForm> getConfigDisplayForms() {
		return plugin.getConfig().getBridgeDisplayForms();
	}

	private void setConfigDisplayForms(List<DisplayForm> displayForms) {
		plugin.getConfig().setBridgeDisplayForms(displayForms);
	}

	@Override
	public String getTitle() {
		return Util.I18N.getString("pref.tree.kmlExport.bridgeRendering");
	}
