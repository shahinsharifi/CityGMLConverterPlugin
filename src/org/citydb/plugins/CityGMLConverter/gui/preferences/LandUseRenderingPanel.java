...
public class LandUseRenderingPanel extends AbstractPreferencesComponent {
...
*/
	...
	public LandUseRenderingPanel(CityKMLExportPlugin plugin) {
		super(plugin);
		initGui();
	}

	private ColladaOptions getConfigColladaOptions() {
		return plugin.getConfig().getLandUseColladaOptions();
	}

	private List<DisplayForm> getConfigDisplayForms() {
		return plugin.getConfig().getLandUseDisplayForms();
	}

	private void setConfigDisplayForms(List<DisplayForm> displayForms) {
		plugin.getConfig().setLandUseDisplayForms(displayForms);
	}

	@Override
	public String getTitle() {
		return Util.I18N.getString("pref.tree.kmlExport.landUseRendering");
	}
