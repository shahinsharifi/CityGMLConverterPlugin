...
*/
	...
	public VegetationRenderingPanel(CityKMLExportPlugin plugin) {
		super(plugin);
		initGui();
	}

	private ColladaOptions getConfigColladaOptions() {
		return plugin.getConfig().getVegetationColladaOptions();
	}

	private List<DisplayForm> getConfigDisplayForms() {
		return plugin.getConfig().getVegetationDisplayForms();
	}

	private void setConfigDisplayForms(List<DisplayForm> displayForms) {
		plugin.getConfig().setVegetationDisplayForms(displayForms);
	}

	@Override
	public String getTitle() {
		return Util.I18N.getString("pref.tree.kmlExport.vegetationRendering");
	}
