 ...
 */
package org.citydb.plugins.CityGMLConverter.common.xlink.resolver;
...
public class XlinkSurfaceGeometry implements DBXlinkResolver {
	...
	public XlinkSurfaceGeometry(Connection batchConn, HeapCacheTable heapTable,
	...
	private void insert(GeometryNode node,
		...
		if (node.geometry != null) {
			...
			if (++memberBatchCounter == Internal.Sqlite_MAX_BATCH_SIZE) {
			...
			long surfaceGeometryId = 0;//resolverManager.getDBId(DBSequencerEnum.SURFACE_GEOMETRY_ID_SEQ);
			...
			if (++parentBatchCounter == Internal.Sqlite_MAX_BATCH_SIZE) {
				psParentElem.executeBatch();
				parentBatchCounter = 0;
			}
